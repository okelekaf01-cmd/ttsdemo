...
export async function saveRecord(record: HistoryRecord, userId: string): Promise<void> {
  let db: IDBDatabase
  ...
  const stored = await encryptRecord(record, userId)
...
export async function getAllRecords(userId: string): Promise<Omit<HistoryRecord, 'audioBlob'>[]> {
  try {
    const db = await openDB()
    const tx = db.transaction(STORE_NAME, 'readonly')
    const all = (await idbReq(tx.objectStore(STORE_NAME).index('createdAt').getAll())) as StoredRecord[]
    const decrypted = await Promise.all(all.filter(r => r.userId === userId).map(decryptRecord))
    return decrypted
...
export async function getRecord(id: string): Promise<HistoryRecord | null> {
  try {
    const db = await openDB()
    const stored = (await idbReq(db.transaction(STORE_NAME, 'readonly').objectStore(STORE_NAME).get(id))) as StoredRecord | undefined
